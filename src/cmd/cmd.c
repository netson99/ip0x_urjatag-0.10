/*
 * $Id: cmd.c 1419 2009-01-19 12:06:06Z kawk $
 *
...
extern cmd_t cmd_spidetectflash;
extern cmd_t cmd_spieraseflash;
extern cmd_t cmd_spiflashmem;
extern cmd_t cmd_spireadflash;
...
cmd_run( chain_t *chain, char *params[] )
{
	...
	for (i = 0; cmds[i]; ++i) {
		if (strcasecmp( cmds[i]->name, params0 ) == 0) {
			int r;
			//printf(_(">>Params: %s, %s\n"), params[0], params[1]);
 run_cmd:
			...
			return r;
		} else if (strncasecmp( cmds[i]->name, params0, len ) == 0) {
			if (pidx == -1)
				pidx = i;
			else
				pidx = -2;
